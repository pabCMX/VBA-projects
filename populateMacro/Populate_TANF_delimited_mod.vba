Attribute VB_Name = "Populate_TANF_delimited_mod"
Sub Populate_TANF_Delimited()
...
Set wb_sch = wb
    ...
    If bis_ind_stop_row > bis_ind_start_row Then
        ...
        wb_bis.Worksheets("Individual").Sort.SortFields.Add Key:=Range( _
            "X" & bis_ind_start_row & ":X" & bis_ind_stop_row), SortOn:=xlSortOnValues, Order:=xlDescending, DataOption:= _
            xlSortNormal
    ...
    Select Case program
        ...
        Case "TANF"
            ...
            wb_sch.Worksheets("TANF Workbook").Range("D20") = _
   ...
   ''********************double check on application date**********************************
       ...
       '     wb_sch.Worksheets("FS Workbook").Shapes("CB 243").OLEFormat.Object.value = 1 'element 166
            ...
            ' Fill in household info in section B
                ...
                If write_row < 23 Then 'only room for 12 names on workbook
                    wb_sch.Worksheets("TANF Workbook").Range("J" & write_row) = lnf(wb_bis.Worksheets("Individual").Range("L" & i)) 'Line Number
                    wb_sch.Worksheets("TANF Workbook").Range("L" & write_row) = Trim(wb_bis.Worksheets("Individual").Range("N" & i)) & " " & _
                    ...
                    wb_sch.Worksheets("TANF Workbook").Range("AC" & write_row) = wb_bis.Worksheets("Individual").Range("J" & i) 'Individual Category
                    temp = wb_bis.Worksheets("Individual").Range("R" & i)
                    wb_sch.Worksheets("TANF Workbook").Range("V" & write_row) = DateSerial(Val(Left(temp, 4)), Val(Mid(temp, 5, 2)), Val(Right(temp, 2)))
                    wb_sch.Worksheets("TANF Workbook").Range("Y" & write_row) = wb_bis.Worksheets("Individual").Range("T" & i) 'Age
                    wb_sch.Worksheets("TANF Workbook").Range("AA" & write_row) = wb_bis.Worksheets("Individual").Range("X" & i) 'Relationship
                    wb_sch.Worksheets("TANF Workbook").Range("AE" & write_row) = wb_bis.Worksheets("Individual").Range("Z" & i) 'SSN
                    If wb_bis.Worksheets("Individual").Range("AD" & i) = "ES" Or wb_bis.Worksheets("Individual").Range("AD" & i) = "EC" Then
                        wb_sch.Worksheets("TANF Workbook").Range("AI" & write_row) = "Yes" 'TANF Received
                    Else
                        wb_sch.Worksheets("TANF Workbook").Range("AI" & write_row) = "No" 'TANF Not Received
                        household_size = household_size + 1
                    End If
                    If wb_bis.Worksheets("Individual").Range("AD" & i) = "ES" Or wb_bis.Worksheets("Individual").Range("AD" & i) = "EC" Then
                        wb_sch.Worksheets("TANF Workbook").Range("AJ" & write_row) = "Yes" 'TANF Received
                    Else
                        wb_sch.Worksheets("TANF Workbook").Range("AJ" & write_row) = "No" 'TANF Not Received
                        household_size = household_size + 1
                    End If
                End If ' write_row < 23
                ...
                If wb_bis.Worksheets("Individual").Range("T" & i) >= 18 And wb_bis.Worksheets("Individual").Range("AB" & i) <> 0 Then
...
'''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
        'wb_sch.Worksheets(review_number).Range("S10") = lnf(wb_bis.Worksheets("Case").Range("K" & i)) 'Grant Group
            'TANF Schedule - Section 4
                i = bis_ind_start_row - 1
                For j = 30 To 44 Step 2
                    i = i + 1
                    wb_sch.Worksheets(review_number).Range("A" & j) = lnf(wb_bis.Worksheets("Individual").Range("L" & i)) 'Line number
                    ...
                    End If
                    Select Case UCase(wb_bis.Worksheets("Individual").Range("X" & i)) 'Relationship to Head of Household
                        Case "X  " 'Head of Household
                          If Val(wb_bis.Worksheets("Individual").Range("T" & i)) <= 19 Then
                            wb_sch.Worksheets(review_number).Range("J" & j) = "02"
                          Else
                            wb_sch.Worksheets(review_number).Range("J" & j) = "01"
                         End If
                        Case "W  ", "H  ", "CLH", "CLW" 'Spouse
                          If wb_bis.Worksheets("Individual").Range("T" & i) <= 19 Then
                            wb_sch.Worksheets(review_number).Range("J" & j) = "04"
                          Else
                            wb_sch.Worksheets(review_number).Range("J" & j) = "03"
                         End If
                        ...
                        Case "A  ", "B  ", "C  ", "GN ", "GNC", "N  ", "NC ", "SR " 'Other related person
                            wb_sch.Worksheets(review_number).Range("J" & j) = "11"
                    End Select
                    wb_sch.Worksheets(review_number).Range("M" & j) = wb_bis.Worksheets("Individual").Range("T" & i) 'Age
                    
                    'Gender
                    If wb_bis.Worksheets("Individual").Range("U" & i) = "F" Then
                        wb_sch.Worksheets(review_number).Range("P" & j) = 2
                    ElseIf wb_bis.Worksheets("Individual").Range("U" & i) = "M" Then
                        wb_sch.Worksheets(review_number).Range("P" & j) = 1
                    Else
                        wb_sch.Worksheets(review_number).Range("P" & j) = ""
                    End If
                    
                    'Race
                    Select Case wb_bis.Worksheets("Individual").Range("V" & i)
                        Case 1
                            wb_sch.Worksheets(review_number).Range("R" & j) = 2
                        Case 3
                            wb_sch.Worksheets(review_number).Range("R" & j) = 5
                        Case 4
                            wb_sch.Worksheets(review_number).Range("R" & j) = 4
                        Case 5
                            wb_sch.Worksheets(review_number).Range("R" & j) = 1
                        'Case 6
                        '    wb_sch.Worksheets(review_number).Range("R" & j) = "12"
                        'Case 7
                        '    wb_sch.Worksheets(review_number).Range("R" & j) = "06"
                        Case 8
                    ...
                    Select Case UCase(wb_bis.Worksheets("Individual").Range("AP" & i)) 'Citizenship
                        Case 1 'US Born
                            wb_sch.Worksheets(review_number).Range("T" & j) = 1
                        Case 2 'Permanent Alien
                            wb_sch.Worksheets(review_number).Range("T" & j) = 2
                        Case 3 'Temporary Alien
                            wb_sch.Worksheets(review_number).Range("T" & j) = 3
                        Case 4 'Refugee
                            wb_sch.Worksheets(review_number).Range("T" & j) = 4
                        Case 5 'Illegal Alien
                            wb_sch.Worksheets(review_number).Range("T" & j) = 6
                        'Case 6 'Refugee Unaccompanied Minor
                            'wb_sch.Worksheets(review_number).Range("T" & j) = 5
                    End Select
                    
                    Select Case wb_bis.Worksheets("Individual").Range("Y" & i) 'Marital Status
                        Case 1 'Single
                            wb_sch.Worksheets(review_number).Range("AK" & j) = 1
                        Case 2 'Married & Living with spouse
                            wb_sch.Worksheets(review_number).Range("AK" & j) = 2
                        Case 4 'Separated-Married-Not living with spouse
                            wb_sch.Worksheets(review_number).Range("AK" & j) = 3
                        Case 5 'Divorced
                            wb_sch.Worksheets(review_number).Range("AK" & j) = 5
                        Case 6 'Widow/Widower
                            wb_sch.Worksheets(review_number).Range("AK" & j) = 4
                        Case 3, 7 'Common Law/Divorced with spousal support
                            wb_sch.Worksheets(review_number).Range("AK" & j) = 9
                     End Select
                     
                    'Education Level - format to two digit code
                    wb_sch.Worksheets(review_number).Range("V" & j) = Format(wb_bis.Worksheets("Individual").Range("AA" & i), "00")
